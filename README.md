...
![System Architecture](https://raw.githubusercontent.com/jlpengtw/aiskill-extractor/main/docs/openclaw-architecture.png)

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                        🤖 OpenClaw AI Gateway Platform                      │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  ┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐    │
│  │   Main Agent    │    │  Sarah Agent    │    │   Agent N...   │    │
│  │  (主要對話代理)  │    │  (分離代理)     │    │   (可擴展)     │    │
│  └────────┬────────┘    └────────┬────────┘    └────────┬────────┘    │
│           │                      │                      │              │
│  ┌────────▼──────────────────────▼──────────────────────▼────────┐    │
│  │                      Gateway (閘道器核心)                        │    │
│  │  • Multi-Agent 協調    • 對話管理    • 工具調度    • 記憶儲存   │    │
│  └────────┬──────────────────────┬──────────────────────┬────────┘    │
│           │                      │                      │              │
│  ┌────────▼────────┐    ┌──────▼──────┐    ┌──────▼──────┐      │
│  │   通訊管道       │    │    工具集    │    │  執行環境    │      │
│  │  • WhatsApp     │    │  • 執行命令   │    │  • Node.js   │      │
│  │  • Telegram     │    │  • 瀏覽器控制 │    │  • LLM API   │      │
│  │  • Discord      │    │  • 記憶管理   │    │    (GPT/     │      │
│  │                 │    │  • 日曆       │    │   Gemini/    │      │
│  │                 │    │  • Gmail      │    │   MiniMax)   │      │
│  │                 │    │  • 網路搜尋   │    │              │      │
